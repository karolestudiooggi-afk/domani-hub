...
async function firecrawlSearch(
  ...
  const res = await fetch(`${SUPABASE_URL}/functions/v1/hub-firecrawl-search`, {
    method: "POST",
    headers: {
...
async function generatePostContent(
  ...
  const res = await fetch(`${SUPABASE_URL}/functions/v1/hub-generate-content`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      apikey: anon,
...
async function fetchIgReferenceContext(apifyToken: string, handles: string[]): Promise<string> {
  ...
  const anon = Deno.env.get("SUPABASE_ANON_KEY")!;
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/hub-social-analytics`, {
      method: "POST",
...
async function handleSchedule(calendarId: string) {
  ...
  for (const post of posts) {
    try {
      ...
      if (needsVisual) {
        ...
        if (fmt === "video" && hfId && hfSecret) {
          try {
            ...
            const r = await fetch(`${SUPABASE_URL}/functions/v1/hub-higgsfield-proxy`, {
              method: "POST",
              headers: {
        ...
        // Arte 100% gpt-image-2: o estilo (quote/infográfico/etc) vai no prompt.
          ...
          try {
            ...
            const r = await fetch(`${SUPABASE_URL}/functions/v1/hub-openai-image`, {
              method: "POST",
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/hub-postforme-proxy`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function handleCheckVisuals(calendarId: string) {
  ...
  for (const post of posts) {
    ...
    try {
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/hub-higgsfield-proxy`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json", apikey: anon, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`,
          "x-higgsfield-api-id": hfId, "x-higgsfield-api-secret": hfSecret,
